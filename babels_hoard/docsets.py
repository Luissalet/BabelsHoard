...
def install_from_data(
    ...
    for i, (page, page_entries) in enumerate(by_page.items()):
        ...
        if progress:
            progress(min(0.95, (i + 1) / total_pages), f"converted {i + 1}/{total_pages} pages")
...
def install(conn, slug: str, progress: Callable[[float, str], None] | None = None) -> dict[str, Any]:
    with httpx.Client() as client:
        ...
        index_data = client.get(f"{DOCUMENTS_BASE}/{slug}/index.json", timeout=60).json()
        if progress:
            progress(0.15, "downloading pages")
        db_data = client.get(f"{DOCUMENTS_BASE}/{slug}/db.json", timeout=120).json()
